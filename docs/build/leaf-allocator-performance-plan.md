...
1. **(Done)** Splits now stream both halves through `LeafAllocator`, so we no
   longer emit `LeafLayout`s or touch `LeafSplitBuilder`. Right-page creation
   reuses the same helper, keeping snapshots hot in the cache.
