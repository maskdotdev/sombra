...
- pager-write-cow: Implement copy-on-write page versioning at commit: writers record new page images with attached commit metadata; old versions remain accessible until oldest reader retires.
  - Decide whether COW happens per dirty page or per logical record; update page cache eviction so it understands multiple generations.
  - Build instrumentation counters to catch regressions (COW pages produced, bytes retained for shadow copies).
- pager-version-tracking: Maintain per-page version chains (ring buffer or append-only table) and add eviction/cleanup pathways triggered after checkpoint + reader quiescence.
  - Define ownership of version metadata (pager vs. WAL manager) so cleanup has a single authority.
  - Include failure-handling plan: if chain metadata corrupts, what invariants during recovery ensure we still reach a consistent checkpoint?
...
- checkpoint-sync: Align checkpoint routine so it can drop WAL segments only after confirming no reader needs the versions they contain.
  ...
  - Add tests simulating long-running readers during checkpoint to ensure we never violate durability or visibility.
...
- [ ] Scope index visibility updates — filtering helpers + deferred maintenance plan.
- [ ] Adapt mutation paths to MVCC — API pluming + invariants.
- [ ] Plan reader tracking metrics — structure and export path.
- [ ] Define vacuum + GC flow — worker pacing + checkpoint hooks.
- [ ] Align checkpoint/WAL pruning — new state machine diagram.
- [ ] Identify unit tests to add — matrix of pager/storage scenarios.
- [ ] Plan end-to-end MVCC tests — concurrent workload descriptions.
- [ ] Plan perf and regression runs — benchmark toggles + reporting scripts.
- [ ] Document path to snapshot iso — list of delta requirements.
- [ ] Define MVCC metrics/logging — metrics list + dashboards.
- [ ] Outline deployment strategy — feature flag rollout + rollback steps.
