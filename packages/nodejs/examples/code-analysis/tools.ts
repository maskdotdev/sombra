import { tool } from "ai";
import type { SombraDB, SombraPropertyValue } from "sombradb";
import { z } from "zod";
...
export function createSombraTools(db: SombraDB) {
	return {
		getNode: tool({
			description:
				"Retrieve a node by its ID with all its properties and labels.",
		...
		getNeighbors: tool({
			description:
				"Get all neighboring nodes connected to a given node (both incoming and outgoing).",
			inputSchema: z.object({
		...
		getIncomingEdges: tool({
			description:
				"Get all incoming edges to a node. Use this to find: what calls a function, what imports a file, what depends on a class.",
			inputSchema: z.object({
		...
		getOutgoingEdges: tool({
			description:
				"Get all outgoing edges from a node. Use this to find: what a function calls, what a file imports, what a class depends on.",
			inputSchema: z.object({
				nodeId: z.number().describe("The ID of the node"),
			}),
			execute: async ({ nodeId }) => {
		...
		traversePath: tool({
			description:
				"Find paths between two nodes using BFS traversal. Useful for dependency analysis and impact analysis.",
			inputSchema: z.object({
			...
			execute: async ({ startId, endId, maxDepth }) => {
				...
				return {
					...
					message: pathExists
						? `Path found from ${startId} to ${endId}`
		...
		queryByLabel: tool({
			description:
				"Find all nodes with a specific label. Examples: find all Functions, all Classes, all Files.",
			inputSchema: z.object({
				label: z
					.string()
					.describe(
						'The label to search for (e.g., "Function", "Class", "File")',
					),
			}),
			execute: async ({ label }) => {
		...
		queryByProperty: tool({
			description:
				'Find nodes by property name and value. Example: find all nodes with name="createUser".',
			inputSchema: z.object({
		...
		findCallChain: tool({
			description:
				"Find the complete call chain from one function to another. Shows all intermediate calls.",
			inputSchema: z.object({
			...
			execute: async ({ startFunctionName, endFunctionName }) => {
				...
				const startNode = allNodes.find(
					(n) => n.properties.name?.value === startFunctionName,
				);
				const endNode = allNodes.find(
					(n) => n.properties.name?.value === endFunctionName,
				);
				...
				if (pathExists) {
					...
					while (current !== startNode.id && visited.has(current)) {
						...
						const callEdge = incomingEdgeIds.find((edgeId: number) => {
							const edge = db.getEdge(edgeId);
							return (
								edge.typeName === "CALLS" && visited.has(edge.sourceNodeId)
							);
						});
		...
		findFunctionCallers: tool({
			description:
				"Find all functions that call a specific function. Perfect for impact analysis.",
			inputSchema: z.object({
				functionName: z
					.string()
					.describe("The function name to find callers for"),
			}),
			execute: async ({ functionName }) => {
				...
				const targetNode = allNodes.find(
					(n) => n.properties.name?.value === functionName,
				);
		...
		findFilesUsingFunction: tool({
			description:
				"Find all files that contain code using a specific function. Traces through CONTAINS and CALLS relationships.",
			inputSchema: z.object({
				...
				for (const edgeId of incomingEdgeIds) {
					...
					if (edge.typeName === "CALLS") {
						...
						while (depth < 10) {
							...
							const containsEdgeId = incomingToCurrentIds.find(
								(eid: number) => {
		...
		getGraphStats: tool({
			description:
				"Get statistics about the graph including node count, edge count, and counts by label/type.",
			inputSchema: z.object({}),
			execute: async () => {
				...
				return {
					nodeCount,
					edgeCount,
					nodesByLabel: Object.entries(nodesByLabel),
					edgesByType: Object.entries(edgesByType),
				};
			},
		}),
	};
