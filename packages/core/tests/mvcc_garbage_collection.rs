...
fn test_gc_removes_old_versions() {
    ...
    let node_id = {
        ...
        tx.commit().unwrap();
        println!("Created node {id} with version 1");
        id
    };
    ...
    for i in 2..=10 {
        ...
        println!(
            "Read node {} (version before update: {:?})",
            node_id,
        ...
        println!("Updated node {node_id} -> {returned_id} with version {i}");
    ...
    assert!(
        stats.chains_scanned > 0,
        "GC should have scanned at least one chain"
    );
    ...
    assert!(
        stats.versions_examined >= 2,
        "GC should have examined at least 2 versions, got {}",
...
fn test_gc_preserves_versions_needed_by_snapshots() {
    ...
    for i in 2..=20 {
        ...
        node.properties.insert(
            "value".to_string(),
            PropertyValue::String(format!("v{}", i)),
        );
...
fn test_manual_gc_trigger() {
    ...
    for i in 1..=10 {
        ...
        let mut node = tx.get_node(node_id).unwrap().unwrap();
        node.properties
            .insert("i".to_string(), PropertyValue::Int(i));
        tx.add_node(node).unwrap();
        tx.commit().unwrap();
    }
    ...
    assert!(
        stats.chains_scanned > 0,
        "Manual GC should have scanned chains"
    );
    assert!(
        stats.versions_examined > 0,
        "Manual GC should have examined versions"
    );
...
fn test_gc_metrics() {
    ...
    assert!(
        stats.versions_examined >= 30,
        "Should have examined at least 30 versions (5 nodes * 6 versions each)"
    );
    assert!(
        stats.versions_reclaimable > 0,
        "Should have found reclaimable versions"
    );
    assert!(
        stats.versions_reclaimed > 0,
