...
fn test_social_graph_concurrent_friend_requests() {
    ...
    thread::scope(|s| {
        for i in 0..20 {
            ...
            s.spawn(move || {
                ...
                edge.properties
                    .insert("timestamp".to_string(), PropertyValue::Int(i as i64));
    ...
    for edge_id in edges.iter() {
        ...
        assert!(edge.is_ok(), "Edge {edge_id} should exist");
    }
    tx.commit().unwrap();
}
...
fn test_e_commerce_concurrent_inventory_updates() {
    ...
    let product_id = {
        ...
        product.properties.insert(
            "name".to_string(),
            PropertyValue::String("Widget".to_string()),
        );
        product
            .properties
            .insert("inventory".to_string(), PropertyValue::Int(100));
                ...
                if let Some(product) = tx.get_node(product_id).unwrap() {
                    if let Some(PropertyValue::Int(inventory)) = product.properties.get("inventory")
                    {
                        if *inventory > 0 {
                            ...
                            purchase
                                .properties
                                .insert("customer_id".to_string(), PropertyValue::Int(i));
                            purchase.properties.insert(
                                "product_id".to_string(),
                                PropertyValue::Int(product_id as i64),
                            );
                            purchase.properties.insert(
                                "inventory_seen".to_string(),
                                PropertyValue::Int(*inventory),
                            );
    ...
    println!("Concurrent purchases: {purchases} successful");
}
...
fn test_banking_concurrent_transfers() {
    ...
    let accounts: Vec<u64> = (0..5)
        .map(|i| {
            ...
            account
                .properties
                .insert("account_number".to_string(), PropertyValue::Int(1000 + i));
            account
                .properties
                .insert("balance".to_string(), PropertyValue::Int(1000)); // $1000 initial
    ...
    thread::scope(|s| {
        for i in 0..20 {
            ...
            s.spawn(move || {
                ...
                transfer.properties.insert(
                    "from".to_string(),
                    PropertyValue::Int(accounts[from_idx] as i64),
                );
                transfer.properties.insert(
                    "to".to_string(),
                    PropertyValue::Int(accounts[to_idx] as i64),
                );
                transfer
                    .properties
                    .insert("amount".to_string(), PropertyValue::Int(amount));
                transfer
                    .properties
                    .insert("transfer_id".to_string(), PropertyValue::Int(i as i64));
...
fn test_wiki_concurrent_page_edits() {
    ...
    let pages: Vec<u64> = (0..3)
        .map(|i| {
            ...
            page.properties.insert(
                "title".to_string(),
                PropertyValue::String(format!("Page {i}")),
            );
            page.properties.insert(
                "content".to_string(),
                PropertyValue::String("Initial content".to_string()),
            );
            page.properties
        ...
        for i in 0..30 {
            ...
            s.spawn(move || {
                ...
                if let Some(page) = tx.get_node(pages[page_idx]).unwrap() {
                    ...
                    edit.properties.insert(
                        "page_id".to_string(),
                        PropertyValue::Int(pages[page_idx] as i64),
                    );
                    edit.properties
                        .insert("editor_id".to_string(), PropertyValue::Int(i as i64));
                    edit.properties.insert(
                        "based_on_version".to_string(),
                        PropertyValue::Int(current_version),
                    );
                    edit.properties.insert(
                        "content".to_string(),
                        PropertyValue::String(format!("Edit by editor {i}")),
                    );
    ...
    for page_id in pages.iter() {
        ...
        if let Some(PropertyValue::Int(version)) = page.properties.get("version") {
            println!("Page {page_id} version: {version}");
    ...
    thread::scope(|s| {
        ...
        for i in 0..5 {
            ...
            s.spawn(move || {
                ...
                new_node
                    .properties
                    .insert("added_by".to_string(), PropertyValue::Int(i as i64));
        ...
        for i in 0..100 {
            ...
            s.spawn(move || {
                ...
                if let Some(counter) = tx.get_node(counter_id).unwrap() {
                    ...
                    increment.properties.insert(
                        "counter_id".to_string(),
                        PropertyValue::Int(counter_id as i64),
                    );
                    increment
                        .properties
                        .insert("thread_id".to_string(), PropertyValue::Int(i));
                    increment
                        .properties
                        .insert("observed_value".to_string(), PropertyValue::Int(current));
        ...
        for i in 0..5 {
            ...
            s.spawn(move || {
                ...
                for j in 0..5 {
                    ...
                    node.properties
                        .insert("created_by".to_string(), PropertyValue::Int(i));
                    node.properties
                        .insert("index".to_string(), PropertyValue::Int(j));
