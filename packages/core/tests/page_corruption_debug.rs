...
fn test_mixed_page_types_with_indexes() {
    ...
    for i in 0..50 {
        ...
        node.properties.insert("age".to_string(), PropertyValue::Int(20 + (i % 30) as i64));
        node.properties.insert("name".to_string(), PropertyValue::String(format!("Person_{}", i)));
        let node_id = db.add_node(node).unwrap();
        node_ids.push(node_id);
    }
    ...
    let results = db.find_nodes_by_property(
        "Person", 
        "age", 
    ...
    for &node_id in &node_ids {
        ...
        assert!(node.is_some(), "Node {} should be readable", node_id);
    }
