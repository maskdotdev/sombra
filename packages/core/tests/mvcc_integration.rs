...
fn test_mvcc_enabled_allocates_snapshot_timestamp() {
    ...
    assert!(ts1 > 0, "Snapshot timestamp should be > 0 when MVCC is enabled");
    ...
    assert!(ts2 > ts1, "Second transaction should get higher timestamp (ts2={}, ts1={})", ts2, ts1);
    tx2.commit().unwrap();
...
fn test_mvcc_timestamp_persists_across_reopen() {
    ...
    // Reopen the database
    {
        ...
        assert!(ts > last_ts, "Timestamp after reopen should be > last timestamp (ts={}, last_ts={})", ts, last_ts);
...
fn test_mvcc_rollback_unregisters_snapshot() {
    ...
    let ts2 = tx2.snapshot_ts();
    assert!(ts2 > _ts1, "New transaction after rollback should get fresh timestamp");
    tx2.commit().unwrap();
...
fn test_mvcc_timestamp_oracle_initialization() {
    ...
    // Reopen - should restore timestamp oracle state
    {
        ...
        assert!(ts > 1, "Timestamp should be restored and continue incrementing");
