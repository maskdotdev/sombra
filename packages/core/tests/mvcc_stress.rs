...
fn test_100_sequential_node_creations() {
    ...
    for (i, node_id) in node_ids.iter().enumerate() {
        ...
        assert_eq!(
            ...
            "Node {node_id} should have index {i}"
        );
    }
    tx.commit().unwrap();
...
fn test_high_write_contention_single_node() {
    ...
    for i in 1..=num_updates {
        ...
        node.properties
            .insert("counter".to_string(), PropertyValue::Int(i as i64));
...
fn test_multiple_hotspot_nodes() {
    ...
    let mut hotspot_ids = Vec::new();
    {
        ...
        for i in 0..num_hotspots {
            ...
            node.properties
                .insert("hotspot_id".to_string(), PropertyValue::Int(i as i64));
            node.properties
                .insert("counter".to_string(), PropertyValue::Int(0));
            let id = tx.add_node(node).unwrap();
            hotspot_ids.push(id);
    ...
    for update_round in 1..=updates_per_hotspot {
        for (hotspot_idx, &node_id) in hotspot_ids.iter().enumerate() {
            ...
            node.properties.insert(
                "counter".to_string(),
                PropertyValue::Int(update_round as i64),
            );
            node.properties.insert(
                "last_hotspot".to_string(),
                PropertyValue::Int(hotspot_idx as i64),
            );
    ...
    for (hotspot_idx, &node_id) in hotspot_ids.iter().enumerate() {
        ...
        assert_eq!(
            node.properties.get("counter"),
            Some(&PropertyValue::Int(updates_per_hotspot as i64))
        );
        assert_eq!(
            node.properties.get("last_hotspot"),
            Some(&PropertyValue::Int(hotspot_idx as i64))
        );
    }
    tx.commit().unwrap();
...
fn test_mixed_read_write_workload() {
    ...
    for tx_num in 0..num_transactions {
        ...
        if tx_num % 2 == 0 {
            ...
            node.properties
                .insert("value".to_string(), PropertyValue::Int(current_value + 1));
            tx.add_node(node).unwrap();
        } else {
...
fn test_long_running_transaction() {
    ...
    for i in 0..operations_per_tx {
        ...
        node.properties
            .insert("operation".to_string(), PropertyValue::Int(i as i64));
        let id = tx.add_node(node).unwrap();
    ...
    for &node_id in &created_nodes {
        let _ = tx
            .get_node(node_id)
            .unwrap()
            .expect("node should exist after commit");
    }
    tx.commit().unwrap();
...
fn test_edge_creation_stress() {
    ...
    for i in 0..num_nodes {
        for j in 0..edges_per_node {
            ...
            tx.add_edge(Edge::new(1, from_id, to_id, "connects"))
                .unwrap();
            tx.commit().unwrap();
            edge_count += 1;
        }
...
fn test_snapshot_isolation_consistency() {
    ...
    for i in 0..num_accounts {
        ...
        let mut node = Node::new(1);
        node.properties
            .insert("account_id".to_string(), PropertyValue::Int(i as i64));
        node.properties
            .insert("balance".to_string(), PropertyValue::Int(initial_balance));
        let id = tx.add_node(node).unwrap();
        account_ids.push(id);
        tx.commit().unwrap();
    }
    ...
    for transfer_num in 0..50 {
        ...
        // Debit from account
        let mut from_node = tx
            .get_node(account_ids[from_idx])
            .unwrap()
            .expect("from account exists");
        ...
        // Credit to account
        let mut to_node = tx
            .get_node(account_ids[to_idx])
            .unwrap()
            .expect("to account exists");
    ...
    for i in 0..num_transactions {
        ...
        match i % 4 {
            0 => {
                ...
                node.properties.insert(
                    "type".to_string(),
                    PropertyValue::String("create".to_string()),
                );
                tx.add_node(node).unwrap();
            }
            1 => {
            ...
            3 => {
                ...
                let mut node = Node::new(1);
                node.properties
                    .insert("verify".to_string(), PropertyValue::Bool(true));
                let id = tx.add_node(node).unwrap();
...
fn test_property_index_mvcc_stress() {
    ...
    for i in 0..num_nodes {
        ...
        node.properties
            .insert("tag".to_string(), PropertyValue::String(tag));
        node.properties
            .insert("index".to_string(), PropertyValue::Int(i as i64));
        tx.add_node(node).unwrap();
        tx.commit().unwrap();
...
fn test_version_chain_growth() {
    ...
    for i in 1..=100 {
        ...
        node.properties
            .insert("timestamp".to_string(), PropertyValue::Int(i * 1000));
        tx.add_node(node).unwrap();
        tx.commit().unwrap();
    }
