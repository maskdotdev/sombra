...
fn profile_transaction_overhead() {
    ...
    println!("   Total: {:.2}ms for {} transactions", duration.as_secs_f64() * 1000.0, iterations);
    ...
    println!("   Overhead: {:.2}µs ({:.1}x slower)\n", avg_us - avg_us_sw, avg_us / avg_us_sw);
}
...
fn profile_node_creation() {
    ...
    for i in 0..iterations {
        ...
        node.properties.insert("data".to_string(), PropertyValue::String("test".to_string()));
    ...
    println!("   - begin_transaction(): {:.2}µs", avg_tx_begin.as_micros() as f64);
    println!("   - add_node():          {:.2}µs", avg_node_add.as_micros() as f64);
    println!("   - commit():            {:.2}µs", avg_commit.as_micros() as f64);
    println!("   - Total:               {:.2}µs\n", 
...
fn profile_version_chain_reads() {
    ...
    for depth in [0, 5, 10, 20, 50] {
        ...
        let current_depth = if depth == 0 { 0 } else { depth - if depth == 5 { 0 } else { 5 } };
        for v in 0..current_depth {
            let mut tx = db.begin_transaction().unwrap();
            if let Some(mut node) = tx.get_node(node_id).unwrap() {
                node.properties.insert("value".to_string(), PropertyValue::Int(v as i64));
                tx.add_node(node).unwrap();
            }
            tx.commit().unwrap();
        }
...
fn profile_wal_overhead() {
    ...
    for i in 0..iterations {
        let mut tx = db.begin_transaction().unwrap();
        let mut node = Node::new(0);
        node.properties.insert("x".to_string(), PropertyValue::Int(i));
        tx.add_node(node).unwrap();
        tx.commit().unwrap();
    }
    ...
    println!("   WAL/Version overhead:  {:.2}µs ({:.1}%)\n", 
             write_avg - empty_avg, 
