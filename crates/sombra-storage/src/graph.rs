...
use sombra_btree::{BTree, BTreeOptions, ValCodec};
use sombra_index::{
    collect_all, IndexDef, IndexRoots, IndexStore, LabelScan, PostingStream, TypeTag,
};
use sombra_pager::{PageStore, ReadGuard, WriteGuard};
use sombra_types::{EdgeId, LabelId, NodeId, PageId, PropId, Result, SombraError, TypeId, VRef};
use sombra_vstore::VStore;

#[cfg(feature = "degree-cache")]
use crate::adjacency::DegreeDir;
use crate::adjacency::{self, Dir, ExpandOpts, Neighbor, NeighborCursor};
use crate::edge::{self, PropPayload as EdgePropPayload, PropStorage as EdgePropStorage};
use crate::node::{self, PropPayload as NodePropPayload, PropStorage as NodePropStorage};
use crate::options::GraphOptions;
use crate::patch::{PropPatch, PropPatchOp};
use crate::profile::{
...
pub struct Graph {
    ...
    storage_flags: u32,
    metrics: Arc<dyn crate::metrics::StorageMetrics>,
...
impl Graph {
    pub fn open(opts: GraphOptions) -> Result<Self> {
        ...
        Ok(Self {
            ...
            metrics: opts
                .metrics
                .unwrap_or_else(|| crate::metrics::default_metrics()),
