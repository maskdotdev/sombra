...
use sombra_index::{collect_all, intersect_k, intersect_sorted, PostingStream};
...
use sombra_types::{LabelId, PropId, Result};
...
fn collect_scan(mut scan: LabelScan<'_>) -> Result<Vec<sombra_types::NodeId>> {
    let mut out = Vec::new();
...
fn collect_stream(stream: &mut dyn PostingStream) -> Result<Vec<sombra_types::NodeId>> {
    let mut out = Vec::new();
...
) -> Result<Vec<sombra_types::NodeId>> {
