...
use sombra_admin::{open_graph, AdminOpenOptions, CheckpointMode, GraphHandle};
use sombra_catalog::Dict;
use sombra_pager::{PageStore, ReadGuard, WriteGuard};
use sombra_storage::{EdgeSpec, NodeSpec, PropEntry, PropValue, PropValueOwned};
use sombra_types::{LabelId, NodeId, PropId, SombraError, StrId, TypeId};
use thiserror::Error;
...
pub enum CliError {
    ...
    Csv(#[from] csv::Error),
    #[error(transparent)]
    Admin(#[from] sombra_admin::AdminError),
    #[error(transparent)]
...
fn materialize_props(
    handle: &GraphHandle,
    read: &ReadGuard,
    props: &[(sombra_types::PropId, PropValueOwned)],
