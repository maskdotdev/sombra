...
fn benchmark_read_with_mmap(enable_mmap: bool, node_count: usize) -> u128 {
    ...
    for i in 0..node_count {
        ...
        node.properties.insert("id".to_string(), sombra::model::PropertyValue::Int(i as i64));
        node.properties.insert("name".to_string(), sombra::model::PropertyValue::String(format!("node_{}", i)));
...
fn main() {
    ...
    for &count in &node_counts {
        ...
        println!("  Without mmap: {}μs ({:.2}ms)", read_without, read_without as f64 / 1000.0);
        
        let read_with = benchmark_read_with_mmap(true, count);
        println!("  With mmap:    {}μs ({:.2}ms)", read_with, read_with as f64 / 1000.0);
