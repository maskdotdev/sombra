...
fn benchmark_persistence(count: usize) {
    ...
    for i in 0..count {
        btree.insert(i as u64, RecordPointer { 
            page_id: i as u32, 
...
fn benchmark_range_queries(count: usize) {
    ...
    let start = Instant::now();
    let results: Vec<_> = btree.range(count as u64 / 4, count as u64 * 3 / 4).collect();
    ...
    println!("  range(25%, 75%): {:?} ({} results)", range_time, results.len());
    println!("  range_from(50%): {:?} ({} results)", range_from_time, from_results.len());
    println!("  range_to(50%):   {:?} ({} results)", range_to_time, to_results.len());
}

fn benchmark_bulk_operations(count: usize) {
    let entries: Vec<_> = (0..count)
        .map(|i| (i as u64, RecordPointer { 
            page_id: i as u32, 
            slot_index: 0 
        }))
        .collect();
    ...
    println!("  batch_remove: {:?} ({} removed)", remove_time, removed.len());
}
...
fn benchmark_custom_btree(count: usize) {
    ...
    let start = Instant::now();
    for i in 0..count {
        custom.insert(i as u64, RecordPointer { 
            page_id: i as u32, 
            slot_index: 0 
        });
    }
...
fn compare_btree_implementations(count: usize) {
    ...
    let start = Instant::now();
    for i in 0..count {
        std_btree.insert(i as u64, RecordPointer { 
            page_id: i as u32, 
            slot_index: 0 
        });
    }
    ...
    let start = Instant::now();
    for i in 0..count {
        custom_btree.insert(i as u64, RecordPointer { 
            page_id: i as u32, 
            slot_index: 0 
        });
    }
    ...
    println!("    insert: {:?} ({:.2}x)", custom_insert, std_insert.as_nanos() as f64 / custom_insert.as_nanos() as f64);
    println!("    lookup: {:?} ({:.2}x)", custom_lookup, std_lookup.as_nanos() as f64 / custom_lookup.as_nanos() as f64);
}
