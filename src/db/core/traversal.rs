...
impl GraphDB {
    ...
    pub fn get_incoming_neighbors(&mut self, node_id: NodeId) -> Result<Vec<NodeId>> {
        ...
        self.incoming_neighbors_cache.insert(node_id, neighbors.clone());
        Ok(neighbors)
    }
    ...
    pub fn bfs_traversal(&mut self, start_node_id: NodeId, max_depth: usize) -> Result<Vec<(NodeId, usize)>> {
        let mut visited = HashSet::new();
