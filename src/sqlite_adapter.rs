...
impl SqliteGraphDB {
    ...
    pub fn bfs_traversal(&mut self, start_node_id: u64, max_depth: usize) -> SqliteResult<Vec<(u64, usize)>> {
    ...
    pub fn get_nodes_by_label(&mut self, label: &str) -> SqliteResult<Vec<u64>> {
        let mut stmt = self.conn.prepare(
            "SELECT id FROM nodes WHERE labels LIKE ?"
        )?;
            ...
            let properties_json = {
                ...
                for (key, value) in &node.properties {
                    let json_value = match value {
                        ...
                        PropertyValue::Bytes(bytes) => serde_json::Value::String(base64::engine::general_purpose::STANDARD.encode(bytes)),
                    };
    ...
    fn properties_to_json(&self, properties: &std::collections::BTreeMap<String, PropertyValue>) -> String {
        let mut map = serde_json::Map::new();
    ...
    fn json_to_property_value(&self, value: &serde_json::Value) -> PropertyValue {
        match value {
            ...
            serde_json::Value::String(s) => {
                ...
                if s.len() % 4 == 0 && s.chars().all(|c| c.is_ascii_alphanumeric() || c == '+' || c == '/' || c == '=') {
                    if let Ok(bytes) = base64::engine::general_purpose::STANDARD.decode(s) {
                        if bytes.len() > 32 { // Heuristic: if it's long, probably bytes
