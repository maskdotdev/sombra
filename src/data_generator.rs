use rand::Rng;
use crate::{Node, Edge, PropertyValue};
...
impl DataGenerator {
    ...
    pub fn generate_social_network(&mut self, num_users: usize, avg_connections: usize) -> (Vec<Node>, Vec<Edge>) {
        ...
        for i in 0..num_users {
            ...
            node.properties.insert("name".to_string(), 
                PropertyValue::String(format!("User{}", i + 1)));
            node.properties.insert("age".to_string(), 
                PropertyValue::Int(self.rng.gen_range(18..65)));
            node.properties.insert("active".to_string(), 
                PropertyValue::Bool(self.rng.gen_bool(0.8)));
            node.properties.insert("join_date".to_string(), 
                PropertyValue::Int(1609459200 + self.rng.gen_range(0..86400 * 365))); // Unix timestamp
            node.properties.insert("score".to_string(), 
            ...
            for _ in 0..num_connections {
                ...
                if target != i {
                    ...
                    if !edges.iter().any(|e: &Edge| 
                        ...
                        let mut edge = Edge::new(edge_id_counter, source_id, target_id, "FRIENDS_WITH");
                        edge_id_counter += 1;
                        edge.properties.insert("since".to_string(), 
                            PropertyValue::Int(1609459200 + self.rng.gen_range(0..86400 * 365)));
                        edge.properties.insert("strength".to_string(), 
    ...
    pub fn generate_product_catalog(&mut self, num_products: usize, num_categories: usize) -> (Vec<Node>, Vec<Edge>) {
        ...
        for i in 0..num_categories {
            ...
            node.properties.insert("description".to_string(), 
                PropertyValue::String(format!("Description for category {}", i + 1)));
            node.properties.insert("level".to_string(), 
        ...
        for i in 0..num_products {
            ...
            node.properties.insert("stock".to_string(), 
                PropertyValue::Int(self.rng.gen_range(0..1000)));
            node.properties.insert("rating".to_string(), 
                PropertyValue::Float(self.rng.gen_range(1.0..5.0)));
            node.properties.insert("available".to_string(), 
            ...
            for _ in 0..num_categories_for_product {
                ...
                edge_id_counter += 1;
                edge.properties.insert("relevance".to_string(), 
    ...
    pub fn generate_knowledge_graph(&mut self, num_entities: usize, num_relationships: usize) -> (Vec<Node>, Vec<Edge>) {
        let mut nodes = Vec::with_capacity(num_entities);
        let mut edges = Vec::new();
        let mut edge_id_counter = 1u64;
        
        let entity_types = vec!["Person", "Organization", "Location", "Event", "Concept"];
        let relationship_types = vec!["WORKS_FOR", "LOCATED_IN", "PARTICIPATES_IN", "RELATED_TO", "KNOWS"];
        ...
        for i in 0..num_entities {
            ...
            node.properties.insert("confidence".to_string(), 
                PropertyValue::Float(self.rng.gen_range(0.5..1.0)));
            node.properties.insert("created_at".to_string(), 
                PropertyValue::Int(1609459200 + self.rng.gen_range(0..86400 * 730))); // 2 years
        ...
        for _ in 0..num_relationships {
            ...
            if source_id != target_id {
                let relationship_type = relationship_types[self.rng.gen_range(0..relationship_types.len())];
                
                let mut edge = Edge::new(edge_id_counter, source_id, target_id, relationship_type);
                edge_id_counter += 1;
                edge.properties.insert("weight".to_string(), 
                    PropertyValue::Float(self.rng.gen_range(0.1..1.0)));
                edge.properties.insert("verified".to_string(), 
