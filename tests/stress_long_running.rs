...
fn stress_test_large_insertion() {
    ...
    println!("Starting insertion of {target_nodes} nodes and {target_edges} edges");
    ...
    for i in 0..target_nodes {
        ...
        if i % 10_000 == 0 && i > 0 {
            ...
            println!("Inserted {i} nodes ({rate:.0} nodes/sec)");
        }
    }
    ...
    for i in 0..target_edges {
        ...
        if i % 100_000 == 0 && i > 0 {
            ...
            let rate = i as f64 / elapsed.as_secs_f64();
            println!("Inserted {i} edges ({rate:.0} edges/sec)");
        }
    }
...
fn stress_test_sustained_throughput() {
    ...
    println!("Running sustained {target_rate} tx/sec for {duration:?}");
    ...
    while start.elapsed() < duration {
        ...
        if last_report.elapsed() >= Duration::from_secs(5) {
            ...
            println!("{elapsed:?} elapsed: {count} transactions ({rate:.0} tx/sec)");
...
fn stress_test_memory_stability() {
    ...
    println!("Testing memory stability over {iterations} operations");
...
fn stress_test_mixed_workload() {
    ...
    for i in 0..operations {
        ...
        if i % 1_000 == 0 && i > 0 {
            println!("Completed {i} mixed operations");
        }
    }
