...
fn init_page(pager: &Pager, write: &mut WriteGuard<'_>, page: PageId, seed: &[u8]) -> Result<()> {
    let meta = pager.meta()?;
...
fn overwrite_payload(write: &mut WriteGuard<'_>, page: PageId, data: &[u8]) -> Result<()> {
    let mut frame = write.page_mut(page)?;
    let buf = frame.data_mut();
